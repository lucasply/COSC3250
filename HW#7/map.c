...
if (*address & 0x1) {
        ulong address = &pagetable - ((vaddr >> 30) & 0x1FF);
    } * @param pagetable  base pagetable
 * @param vaddr      virtual address at which the page will reside
 * @param paddr      physical frame that will be mapped at the virtual address * @param attr       any attributes to set on the page table entry
 * @return           OK if successful, otherwise a SYSERR
 */
syscall mapPage(pgtbl pagetable, ulong vaddr, ulong paddr, int attr)
{
 ...
 */
    ...
    * Otherwise create the page by calling pgalloc().  Make sure to setup the     * page table entry accordingly. Call sfence_vma once finished to flush TLB    * Once you've traversed all three levels, set the attributes (attr) for
    * the leaf page (don't forget to set the valid bit!)
    */

    // Level 2
    ulong address = &pagetable + (((vaddr >> 30) & 0x1FF) * PTE_SIZE);

    // Level 1
    if (*address & 0x1) {
        address = ((*address >> 10) & 0x7FFFFFFFFFF) + (((vaddr >> 21) & 0x1FF) * PTE_SIZE);
    }

    // Level 0
    if (*address & 0x1) {
        address = ((*address >> 10) & 0x7FFFFFFFFFF) + (((vaddr >> 12) & 0x1FF) * PTE_SIZE);
    }

    paddr = ((*address >> 10) & 0x7FFFFFFFFFF) + (vaddr & 0xFFF); // Check this line

    // This part isn't complete
